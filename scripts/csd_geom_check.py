...
if __name__ == "__main__":
    ...
    for directory_path in tqdm(directory_paths, desc='Methods', unit='method'):
        ...
        df.to_csv(os.path.join(RESULTS_DIR, f'{method_name}_mogul_results.csv'), index=False)
        summary_rows.append(summarise_mogul(df, method_name))

    output_overall = pd.DataFrame(summary_rows)
    output_overall.to_csv(os.path.join(RESULTS_DIR, 'all_methods_mogul_summary.csv'), index=False)
